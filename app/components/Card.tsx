...
function CardContent({ children, condensed, additionalStyles }: CardContentProps) {
  return (
    <div className={`${condensed ? "p-3" : "p-4"} shadow-md rounded-md bg-[var(--surface-variant)] text-[var(--on-surface-variant)] hover:bg-[var(--secondary-container)] hover:text-[var(--on-secondary-container)] transition-colors duration-300 ${additionalStyles}`}>
      {children}
    </div>
  )
}